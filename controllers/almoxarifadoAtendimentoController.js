...
const registrarEntregaItem = async (req, res) => {
    try {
        ...
        if (!codigoBarras || !quantidade) { return res.status(400).json({ error: "Dados de entrega incompletos." }); }

        const pedido = await AtendimentoService.registrarEntrega(id, codigoBarras, { quantidade });
