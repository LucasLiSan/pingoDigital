	...
	try {
		...
		for (const mat of materiais) {
			...
			if (mat.medidas?.largura_cm || mat.medidas?.altura_cm || mat.medidas?.comprimento_cm) {
				...
				tamanho = `(L)${l}x(A)${a}x(C)${c}cm`;
			}

			if (mat.peso_gramas) { tamanho += (tamanho ? " | " : "") + `${mat.peso_gramas} g`; }

			div.innerHTML = `
				<div class="itemCodBar">
					${mat.codigoBarras}
				</div>
				<div class="itemQtd">
					${mat.quantidadeAtual}
				</div>
				<div class="itemStatus" style="color:${statusColor}">
					${mat.status}
				</div>
				<div class="itemTitle">
					${mat.nome}
				...
				<div class="itemTime">
					${mat.validade ? new Date(mat.validade).toLocaleDateString("pt-BR") : "-"}
				</div>
				<div class="itemSize">
					${tamanho || "-"}
				</div>
...
function verItem(codigo) { alert("Visualizar: " + codigo); }
function editarItem(codigo) { alert("Editar: " + codigo); }
...
async function carregarPedidosAtendimento() {
	try {
		...
		for (const pedido of pedidos) {
			...
			const materiaisComLocal = (
				await Promise.all(pedido.materiais.map(async (item) => {
					const resMat = await fetch(`/materiais/${item.codigoBarras}`);
					if (!resMat.ok) return null;
...
async function carregarPedidosParaModal(numPedido) {
	try {
		...
		for (const item of pedido.materiais) {
			...
			const campoVariacoes = temCorOuTamanho ? ` ${item.corSelecionada ? `<p class="colorSize">Cor: <input type="color" value="${item.corSelecionada}" disabled></p>` : ""} ${item.tamanhoSelecionado ? `<p>Tamanho: ${item.tamanhoSelecionado}</p>` : ""} ` : `<p>N/A</p>`;
			const div = document.createElement("div");
		...
		if (codigoBarras) {
			...
			if (checkRes.ok) {
				...
				await fetch(`/materiais`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ ...dados, codigoBarras, entradas: [entrada] })
				});
			...
			await fetch(`/materiais`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ ...dados, entradas: [entrada] })
			});
