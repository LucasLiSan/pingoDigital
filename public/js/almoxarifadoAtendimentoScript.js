  ...
  try {
    ...
    for (const pedido of pedidos) {
      ...
      const header = `
        ...
        <div class="descricaoPedido">
          <div class="origemPedido">
            <span class="origem">
                ${pedido.solicitante?.setor && pedido.solicitante?.nome 
                    ? `${pedido.solicitante.setor} - ${pedido.solicitante.nome}` 
                    : "-"}
            </span>
      ...
      for (const item of pedido.materiais) {
        ...
        const localizacao = material?.localizacao
          ? `ARMARIO ${material.localizacao.armario} - PRATELEIRA ${material.localizacao.prateleira}`
          : "N/D";
        ...
        const entregasHtml = (item.entregas || []).map(e => {
          ...
          return `<span class="qtdEntregue">${e.quantidade}</span>
    ...
    console.error("Erro ao carregar pedidos de atendimento:", error);
  }
}
